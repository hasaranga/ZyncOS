...
exports.ZyncLoop = exports.ZyncSetup = void 0;
const ImGui = require("../imgui/imgui.js");
const indexeddb_fs_1 = require("indexeddb-fs");
const zynctheme_1 = require("./zynctheme");
...
const show_system_editor = STATIC(UNIQUE("show_system_editor#10001"), false);
const show_system_about = STATIC(UNIQUE("show_system_about#10002"), false);
const show_system_logs = STATIC(UNIQUE("show_system_logs#10003"), false);
let systemEditorOpenedFileName = "";
...
let systemTime = "";
let systemTimeTextWidth;
let systemTimeLastUpdate = 0;
let systemLog = new ImGui.Vector();
let rootDirData;
let fileSystem;
let shouldShowNewFileInputBox = false;
let shouldCloseNewFileInputPopUp = false;
let shouldShowDeleteFilePopUp = false;
const runningOnMobile = (0, zynctheme_1.isMobile)();
let appCollection = Array();
function registerApp(appName, fileName) {
    return __awaiter(this, void 0, void 0, function* () {
        if (yield fileSystem.exists("root/" + fileName)) {
            let code = yield fileSystem.readFile("root/" + fileName);
            let appVisible = STATIC(UNIQUE(fileName), false);
...
function clearFileSystem() {
    return __awaiter(this, void 0, void 0, function* () {
        rootDirData = yield fileSystem.readDirectory("root");
        for (let file of rootDirData.files) {
            yield fileSystem.removeFile(file.fullPath);
        }
        rootDirData = yield fileSystem.readDirectory("root");
    });
}
...
function exportFileSystem() {
    return __awaiter(this, void 0, void 0, function* () {
        let fileCollection = Array();
        for (let file of rootDirData.files) {
            let fileContent = yield fileSystem.readFile(file.fullPath);
...
function createFileSystemFromJSONText(text) {
    return __awaiter(this, void 0, void 0, function* () {
        let fileCollection = JSON.parse(text);
        yield clearFileSystem();
        for (let item of fileCollection) {
            yield fileSystem.writeFile("root/" + item.fileName, atob(item.content));
        }
        yield readFileSystem();
        yield runBootJS();
    });
...
function importFileSystem() {
    return __awaiter(this, void 0, void 0, function* () {
        const text = yield createUploadAndReadFile();
        if (text) {
            yield createFileSystemFromJSONText(text.toString());
...
function readFileContentToBuffer(path) {
    return __awaiter(this, void 0, void 0, function* () {
        const fileContent = yield fileSystem.readFile(path);
        systemEditorTextBuffer = new ImGui.StringBuffer(MAX_TEXT_FILE_SIZE, fileContent);
    });
}
function saveBufferToFile(path) {
    return __awaiter(this, void 0, void 0, function* () {
        yield fileSystem.writeFile(path, systemEditorTextBuffer.buffer);
    });
}
function runBootJS() {
    return __awaiter(this, void 0, void 0, function* () {
        let source = yield fileSystem.readFile("root/boot.js");
...
function updateIfApplication(fileName) {
    return __awaiter(this, void 0, void 0, function* () {
...
function ZyncSetup() {
    ...
    fileSystem = (0, indexeddb_fs_1.createFs)({});
    loadFileSystem();
    (0, zynctheme_1.SetupZyncTheme)();
}
exports.ZyncSetup = ZyncSetup;
function ZyncLoop() {
    ...
    if (show_system_editor.value) {
        ...
        ImGui.SetNextWindowPos(new ImGui.Vec2((screenWidth - systemEditorInitialWidth) / 2, (screenHeight - systemEditorInitialHeight) / 2), ImGui.Cond.FirstUseEver);
        ...
        if (ImGui.Begin(systemEditorTitle, show_system_editor.access, ImGui.ImGuiWindowFlags.NoSavedSettings | ImGui.ImGuiWindowFlags.MenuBar)) {
            ...
            ImGui.InputTextMultiline("##source", systemEditorTextBuffer, MAX_TEXT_FILE_SIZE, new ImGui.Vec2(-FLT_MIN, -FLT_MIN), ImGui.InputTextFlags.AllowTabInput);
            ...
            if (ImGui.BeginPopupModal("Delete File...", null, ImGui.WindowFlags.AlwaysAutoResize)) {
                ...
                ImGui.SetItemDefaultFocus();
                ImGui.SameLine();
                if (ImGui.Button("No", new ImGui.Vec2(120, 0))) {
                    ImGui.CloseCurrentPopup();
                }
                ImGui.EndPopup();
            }
